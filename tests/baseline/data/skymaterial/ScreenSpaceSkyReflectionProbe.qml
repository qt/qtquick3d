...
// Regression guard: screen-space background (ScreenSpaceFull) with image-based lighting
// turned OFF, plus a ReflectionProbe and reflective spheres. In the screen-space modes the
// radiance cube is only generated when something consumes it; a reflection probe is one such
// consumer. If the cube is (incorrectly) skipped here, the probe has no sky input and the
// metallic spheres reflect clear color instead of the sky. With the cube generated, the
// spheres reflect the procedural sky.
Rectangle {
    id: root
    width: 800
    height: 480
    ...
    SkyMaterial {
        ...
        skyboxMode: SkyMaterial.ScreenSpaceFull
        enableIBL: false
    ...
    View3D {
        ...
        ReflectionProbe {
            ...
            boxSize: Qt.vector3d(4000, 4000, 4000)
