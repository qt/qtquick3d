...
// Same sky as BasicSky.qml, but the visible background is evaluated directly on
// screen (skyboxMode: ScreenSpaceFull) instead of being sampled from the IBL cube.
// IBL stays enabled, so the reflective spheres still pick up the radiance cube.
Rectangle {
    id: root
    width: 800
    height: 480
