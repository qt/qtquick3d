...
void QQuick3DParticleWander::affectParticle(const QQuick3DParticleData &sd, QQuick3DParticleDataCurrent *d, float time)
{
    ...
    if (shouldAffect(sd, particleTimeS)) {
        ...
        if (!qFuzzyIsNull(m_uniqueAmount.x()) && !qFuzzyIsNull(m_uniquePace.x())) {
            ...
            float paceVariation = 1.0f + m_uniquePaceVariation - 2.0f * QPRand::get(sd.index, QPRand::WanderXPV) * m_uniquePaceVariation;
            float amountVariation = 1.0f + m_uniqueAmountVariation - 2.0f * QPRand::get(sd.index, QPRand::WanderXAV) * m_uniqueAmountVariation;
            float startPace = QPRand::get(sd.index, QPRand::WanderXPS) * pi2;
        ...
        if (!qFuzzyIsNull(m_uniqueAmount.y()) && !qFuzzyIsNull(m_uniquePace.y())) {
            // Values between  1.0 +/- variation
            float paceVariation = 1.0f + m_uniquePaceVariation - 2.0f * QPRand::get(sd.index, QPRand::WanderYPV) * m_uniquePaceVariation;
            float amountVariation = 1.0f + m_uniqueAmountVariation - 2.0f * QPRand::get(sd.index, QPRand::WanderYAV) * m_uniqueAmountVariation;
            float startPace = QPRand::get(sd.index, QPRand::WanderYPS) * pi2;
        ...
        if (!qFuzzyIsNull(m_uniqueAmount.z()) && !qFuzzyIsNull(m_uniquePace.z())) {
            // Values between  1.0 +/- variation
            float paceVariation = 1.0f + m_uniquePaceVariation - 2.0f * QPRand::get(sd.index, QPRand::WanderZPV) * m_uniquePaceVariation;
            float amountVariation = 1.0f + m_uniqueAmountVariation - 2.0f * QPRand::get(sd.index, QPRand::WanderZAV) * m_uniqueAmountVariation;
            float startPace = QPRand::get(sd.index, QPRand::WanderZPS) * pi2;
