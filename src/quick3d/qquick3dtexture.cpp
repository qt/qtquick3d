...
bool QQuick3DTexture::effectiveFlipV() const
{
...
QSSGRenderGraphObject *QQuick3DTexture::updateSpatialNode(QSSGRenderGraphObject *node)
{
    ...
    if (m_dirtyFlags.testFlag(DirtyFlag::TransformDirty)) {
        ...
        // imageNode->m_flipV is set later on below
    ...
    // Calculate and forward the effective flipV value. This may depend on
    // various source or sourceItem related parameters, not just on m_flipV
    // itself, so do this always, regardless of m_dirtyFlags.
    const bool flipV = effectiveFlipV();
    if (imageNode->m_flipV != flipV) {
        imageNode->m_flipV = flipV;
        imageNode->m_flags.setFlag(QSSGRenderImage::Flag::TransformDirty);
    }
