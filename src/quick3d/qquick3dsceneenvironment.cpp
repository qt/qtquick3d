...
/*!
    ...
    \value SceneEnvironment.SkyMaterial
        The scene will not be cleared, but instead a SkyBox or Skydome will be rendered. The SkyBox
        is sampled from the cubemap generated by the SkyMaterial defined in the skyMaterial property.
        (Since Qt 6.12)
...
/*!
    \qmlproperty QtQuick3D::SkyMaterial QtQuick3D::SceneEnvironment::skyMaterial

    This property defines a procedural, GPU-driven sky model that is used for
    IBL and as background to the scene.

    \since 6.12
    \sa backgroundMode, lightProbe
*/
QQuick3DSkyMaterial *QQuick3DSceneEnvironment::skyMaterial() const
{
...
void QQuick3DSceneEnvironment::setSkyMaterial(QQuick3DSkyMaterial *skyMaterial)
{
    if (m_skyMaterial == skyMaterial)
        return;

    QQuick3DObjectPrivate::attachWatcher(this, &QQuick3DSceneEnvironment::setSkyMaterial, skyMaterial, m_skyMaterial);

    m_skyMaterial = skyMaterial;
    emit skyMaterialChanged();
    update();
}
